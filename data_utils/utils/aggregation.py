...
HEADER_EXP = "\t".join(["id", "wav_path", "label", "emotion"])
...
def read_data_markup(dataset_path: Path) -> List[MarkupDataclass]:
    markup_data = []
    with open(
...
def agg_data_to_file(file_path: Path, agg_data: List[AggDataclass]) -> None:
    with open(
...
def exp_data_to_file(file_path: Path, exp_data: List[DataForExp]) -> None:
    with open(
        file_path.parent / (file_path.stem + ".tsv"), "w", encoding="utf-8"
    ) as file:
    ...
    for row in markup_data:
        ...
        if row.hash_id in aggregated_data_dict:
            good_agg_row = AggDataclass(
                hash_id=row.hash_id,
                audio_path=str(Path(data_path, dataset, row.audio_path)),
...
def aggregate_data(
    data_path: Path, out_path: Path, dawidskene_threshold: float
) -> None:
